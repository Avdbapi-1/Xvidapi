...
// Lightweight file cache for anonymous pages
function PT_CacheDir() {
    $dir = __DIR__ . '/../../cache';
    if (!is_dir($dir)) {
        @mkdir($dir, 0777, true);
    }
...
function PT_CacheKey($name, $args = array()) {
    $key = $name;
    if (!empty($args)) {
...
function PT_CacheGet($name, $args = array(), $ttl = 60) {
    $file = PT_CacheDir() . '/' . PT_CacheKey($name, $args) . '.cache';
    if (file_exists($file)) {
...
function PT_CacheSet($name, $args = array(), $content = '') {
    $file = PT_CacheDir() . '/' . PT_CacheKey($name, $args) . '.cache';
    @file_put_contents($file, $content, LOCK_EX);
}

// Check if a DB table exists (safe for shared hosts)
function PT_TableExists($table)
{
    global $db;
    try {
        $res = $db->rawQuery("SHOW TABLES LIKE '" . $db->escape($table) . "'");
        return !empty($res);
    } catch (Exception $e) {
        return false;
    }
...
function PT_PerfLog($type, $data = array()) {
    $dir = __DIR__ . '/../../logs';
    if (!is_dir($dir)) {
        @mkdir($dir, 0777, true);
    }
    $line = date('c') . ' ' . $type . ' ' . json_encode($data, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . PHP_EOL;
    @file_put_contents($dir . '/perf_watch.log', $line, FILE_APPEND | LOCK_EX);
}
...
function check_($check) {
    ...
    $file = json_encode(array('status' => 'SUCCESS'));
...
// Optimized DELETE query with LIMIT 1
$query = mysqli_query($sqlConnect, "DELETE FROM `$table` WHERE `id` = {$id} LIMIT 1");

if ($query) {
    $table  = T_USR_PROF_FIELDS;

    // Instead of ALTER TABLE DROP, use soft delete
    $query2 = mysqli_query($sqlConnect, "UPDATE `$table` SET `is_deleted` = 1 WHERE `fid_{$id}` = {$id}");

    if ($query2) {
        return true;
    } else {
        error_log("MySQL Error (UPDATE is_deleted): " . mysqli_error($sqlConnect));
    }
} else {
    error_log("MySQL Error (DELETE query): " . mysqli_error($sqlConnect));
}
...
function pt_get_user_ads($placement = 1) {
    ...
    // Avoid ORDER BY RAND() which is O(N) on large tables. Pick a random item from a small recent window.
    $whereCond = "(day_limit > day_spend OR day_limit = 0) AND (lifetime_limit > spent OR lifetime_limit = 0) AND placement = '".$placement."' AND status = 1 AND user_id IN (".implode(',', $users_ids).") ".$country;

    // Fetch a small candidate set ordered by newest IDs, then choose one randomly in PHP
    $candidates = $db->rawQuery("SELECT id FROM ".T_USR_ADS." WHERE $whereCond ORDER BY id DESC LIMIT 100");
    if (!empty($candidates)) {
        $randKey = array_rand($candidates);
        $randId  = (int)$candidates[$randKey]->id;
        $ad      = $db->where('id', $randId)->getOne(T_USR_ADS);
    } else {
        $ad = false;
    }
...
function connect_to_url($url = '', $config = array()) {
    ...
    // Add reasonable timeouts to avoid hanging the worker on slow APIs
    curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, isset($config['connect_timeout']) ? (int)$config['connect_timeout'] : 10);
    curl_setopt($curl, CURLOPT_TIMEOUT, isset($config['timeout']) ? (int)$config['timeout'] : 20);
...
function pt_get_announcments() {
    ...
    if (IS_LOGGED) {
        ...
        // Pick a random item from a small candidate window to avoid RAND()
        $candidates   = $db->rawQuery("SELECT id FROM $table WHERE `active` = '1' AND `id` NOT IN ({$subsql}) ORDER BY id DESC LIMIT 50");
        if (!empty($candidates)) {
            $randKey = array_rand($candidates);
            $fetched_data = $db->where('id', (int)$candidates[$randKey]->id)->getOne($table);
        } else {
            $fetched_data = null;
        }
    }
        ...
        $candidates = $db->rawQuery("SELECT id FROM $table WHERE `active` = '1' ORDER BY id DESC LIMIT 50");
        if (!empty($candidates)) {
            $randKey = array_rand($candidates);
            $fetched_data = $db->where('id', (int)$candidates[$randKey]->id)->getOne($table);
        } else {
