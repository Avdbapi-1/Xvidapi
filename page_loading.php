...
<?php
// Emit server-side performance headers for AJAX loads
if (!headers_sent()) {
    $pt_gen_ms = round((microtime(true) - (isset($pt->bench_start) ? $pt->bench_start : microtime(true))) * 1000, 1);
    $pt_mem_mb = round(memory_get_peak_usage(true) / (1024*1024), 1);
    header('X-PT-Gen-Time: ' . $pt_gen_ms . 'ms');
    header('X-PT-Memory: ' . $pt_mem_mb . 'MB');
}
