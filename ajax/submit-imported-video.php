...
if(!isset($_POST['video_type']) && isset($_POST['video-type'])) {
    $_POST['video_type'] = $_POST['video-type'];
}


$getID3 = new getID3;

// Decode any HTML entities coming from scrapers/APIs to avoid double-encoding later
if (!function_exists('pt_decode_import_str')) {
    function pt_decode_import_str($str) {
        if (!is_string($str)) { return $str; }
        // Normalize to UTF-8 and decode HTML entities like &amp; &quot; etc.
        $str = html_entity_decode($str, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        // Collapse accidental multiple encodings like &amp;amp; -> &amp;
...
// Map imported author/studio name to a user_id, creating account if missing
if (!function_exists('pt_author_to_user_id')) {
    function pt_author_to_user_id($raw, $provider = 'ext', $fallback_user_id = 0) {
        global $db;
        $name = pt_decode_import_str((string)$raw);
        if (empty($name)) {
            return (int)$fallback_user_id;
        }
        $lower = strtolower($name);
        if (in_array($lower, ['updating','unknown','n/a','na'])) {
            return (int)$fallback_user_id;
        }
        $slug_us = preg_replace('/[^a-z0-9]+/i','_', $lower);
        $slug_us = trim($slug_us, '_');
        $slug_cc = preg_replace('/[^a-z0-9]+/i','', $lower);
        ...
        foreach ($variants as $candidate) {
            ...
            $take = $db->where('username', $candidate)->getOne(T_USERS);
            if (!empty($take) && !empty($take->id)) { return (int)$take->id; }
        }
        $slug = $slug_us; if (strlen($slug) < 5) { $slug = 'ch_' . substr(md5($lower), 0, 8); }
        // Build unique username/email
        $username = $slug; $i = 0;
        while (PT_UsernameExists($username)) { $i++; $username = $slug.$i; }
        $email = $username.'@'.$provider.'.local'; $i = 0;
        while (PT_UserEmailExists($email)) { $i++; $email = $username.$i.'@'.$provider.'.local'; }
        // Create user
        $password_hashed = password_hash(bin2hex(random_bytes(6)), PASSWORD_DEFAULT);
        $insert_data = array(
            'username'    => $username,
        ...
        return !empty($new_id) ? (int)$new_id : (int)$fallback_user_id;
    }
}
$trailer = '';
// Only require a title at this stage; fill other fields if missing
...
elseif ($pt->config->embed_videos == 'on' && $_POST['video-type'] == 'embed' && empty($_FILES['thumbnail']) && empty($_POST['thumbnail-image'])) {
...
if (empty($error)) {
    // Owner resolve: default current user, but honor "author" or "owner_username" if provided
    $owner_id = $user->id;
    if (!empty($_POST['author'])) {
        $owner_id = pt_author_to_user_id($_POST['author'], 'ext', $owner_id);
    } elseif (!empty($_POST['owner_username'])) {
        $try = $db->where('username', PT_Secure($_POST['owner_username']))->getOne(T_USERS);
        if (!empty($try) && !empty($try->id)) { $owner_id = (int)$try->id; }
    }
    // Clean imported strings to prevent double-encoding (e.g., &amp;)
    if (isset($_POST['title'])) { $_POST['title'] = pt_decode_import_str($_POST['title']); }
    if (isset($_POST['description'])) { $_POST['description'] = pt_decode_import_str($_POST['description']); }
    if (isset($_POST['tags'])) { $_POST['tags'] = pt_decode_import_str($_POST['tags']); }
    if (isset($_POST['stars'])) { $_POST['stars'] = pt_decode_import_str($_POST['stars']); }
    if (isset($_POST['producer'])) { $_POST['producer'] = pt_decode_import_str($_POST['producer']); }
    if (isset($_POST['country'])) { $_POST['country'] = pt_decode_import_str($_POST['country']); }

    // Normalize optional fields
    if (!isset($_POST['description'])) { $_POST['description'] = ''; }
    if (!isset($_POST['tags']) || $_POST['tags'] === '') { $_POST['tags'] = $_POST['title']; }
    ...
    // Ensure title fits DB schema (videos.title varchar(100))
    if (function_exists('mb_substr')) {
        $video_title = mb_substr($video_title, 0, 100, 'UTF-8');
    } else {
        $video_title = substr($video_title, 0, 100);
    }
    ...
    $data_insert      = array(
        ...
        'stars' => isset($_POST['stars']) ? (function_exists('mb_substr') ? mb_substr($_POST['stars'], 0, 200, 'UTF-8') : substr($_POST['stars'], 0, 200)) : "",
    ...
    if ($video_ok == true) {
	    ...
	    if ($insert) {
            ...
            // Collect debug info
            $debugErr  = method_exists($db,'getLastError') ? $db->getLastError() : '';
            $debugNo   = method_exists($db,'getLastErrno') ? $db->getLastErrno() : '';
            $debugSql  = method_exists($db,'getLastQuery') ? $db->getLastQuery() : '';
            // Log to file for admins
            $logDir = __DIR__ . '/../logs';
            if (!is_dir($logDir)) { @mkdir($logDir, 0777, true); }
            @file_put_contents($logDir . '/import_error.log', date('c') . "\tERRNO=".$debugNo."\tERR=".$debugErr."\tSQL=".$debugSql."\tDATA=".json_encode($data_insert)."\n", FILE_APPEND);
            $data = array(
                'status' => 400,
                'message' => $error_icon . ' DB insert failed: ' . $debugErr,
                'errno' => $debugNo
            );
            header('Content-Type: application/json');
            echo json_encode($data);
