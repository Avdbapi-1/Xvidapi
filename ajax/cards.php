...
if ($first == 'search'){
	if (!empty($_POST['search_value'])) {
        ...
        // Try FULLTEXT first for better performance
        $search_result = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE MATCH (title,description,tags) AGAINST ('$search_value' IN NATURAL LANGUAGE MODE) AND privacy = 0 AND user_id = ".$pt->user->id." ORDER BY time DESC LIMIT 10");
        if (empty($search_result)) {
            $search_result = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE (title LIKE '%$search_value%' OR tags LIKE '%$search_value%' OR description LIKE '%$search_value%') AND privacy = 0 AND user_id = ".$pt->user->id." ORDER BY time DESC LIMIT 10");
        }
		if (!empty($search_result)) {
			$html = '';
			foreach ($search_result as $key => $search) {
                $search = PT_GetVideoByID($search, 0, 0, 0, 0, 1);
