<?php 
	...
	if (!empty($last_id) && !empty($video_id)) {
        ...
        $video_title    = $video_data->title;
            $sql_query      = "
                SELECT v.* FROM `$t_videos` v
                WHERE MATCH (v.title) AGAINST ('".$video_title."' IN NATURAL LANGUAGE MODE)
                AND v.id <> '".$video_id."' AND v.user_id NOT IN (".implode(',', $pt->blocked_array).")
                AND v.id < '".$last_id."'
                AND v.privacy = 0
                ORDER BY MATCH (v.title) AGAINST ('".$video_title."' IN NATURAL LANGUAGE MODE) DESC, v.id DESC
                LIMIT 20";
