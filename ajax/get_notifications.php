...
if (IS_LOGGED == false) {
    ...
    // Always respond fast for 'new' type to avoid holding session locks
    if ($type == 'new') {
        if (is_numeric($notif_set)) {
            $new_count = intval($notif_set);
        } else {
            $new_count = 0;
        }
        ...
        // Include unread messages counter in the fast response (optional UI update)
        $data['count_messages'] = $db->where('to_id', $user->id)->where('seen', 0)->getValue(T_MESSAGES, "COUNT(*)");

        // Respond immediately and avoid holding session lock, then continue background tasks
        if (!headers_sent()) {
            PT_RunInBackground($data);
        }
        // Continue without exit to allow background housekeeping below if needed
    }
        ...
        // For any other type, respond quickly with 304 to free the session
        $data['status'] = 304;
        $data['count_messages'] = $db->where('to_id', $user->id)->where('seen', 0)->getValue(T_MESSAGES, "COUNT(*)");
        if (!headers_sent()) {
            PT_RunInBackground($data);
        }
        // Continue with background tasks (coinpayments / activity) without blocking the client
    }
...
// Already added to the fast responses above; keep here for non-flushed code paths if any
$data['count_messages'] = isset($data['count_messages']) ? $data['count_messages'] : $db->where('to_id', $user->id)->where('seen', 0)->getValue(T_MESSAGES, "COUNT(*)");
// user active
