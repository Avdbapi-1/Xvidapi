<?php
// Include functions for image download
...
// Helpers for provider imports
if (!function_exists('pt_decode_import_str')) {
    function pt_decode_import_str($str) {
        if (!is_string($str)) { return $str; }
        $str = html_entity_decode($str, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        while (strpos($str, '&amp;') !== false) {
            $prev = $str; $str = str_replace('&amp;', '&', $str); if ($str === $prev) { break; }
        }
        return trim($str);
    }
}
if (!function_exists('pt_author_to_user_id')) {
    function pt_author_to_user_id($raw, $provider = 'ext', $fallback_user_id = 1) {
        global $db;
        $name = pt_decode_import_str($raw);
        if (empty($name) || in_array(strtolower($name), ['updating','unknown','n/a','na'])) {
            return (int)$fallback_user_id;
        }
        $base = strtolower($name);
        $slug_us = preg_replace('/[^a-z0-9]+/i', '_', $base);
        $slug_us = trim($slug_us, '_');
        $slug_cc = preg_replace('/[^a-z0-9]+/i', '', $base); // concatenated
        $variants = array_unique(array_filter([
            $slug_us,
            $slug_cc,
            str_replace('_','-',$slug_us),
            str_replace('_','',$slug_us),
        ]));
        foreach ($variants as $candidate) {
            if (strlen($candidate) < 3) { continue; }
            $user = $db->where('username', $candidate)->getOne(T_USERS);
            if (!empty($user) && !empty($user->id)) { return (int)$user->id; }
        }
        // Ensure minimum username length
        $slug = $slug_us; if (strlen($slug) < 5) { $slug = 'ch_' . substr(md5($base), 0, 8); }
        // Create new user
        $username = $slug; $suffix = 0;
        while (PT_UsernameExists($username)) { $suffix++; $username = $slug . $suffix; }
        $emailDomain = $provider . '.local';
        $email = $username . '@' . $emailDomain;
        $suffix = 0;
        while (PT_UserEmailExists($email)) { $suffix++; $email = $username . $suffix . '@' . $emailDomain; }
        ...
        $new_id = $db->insert(T_USERS, $insert_data);
        if (!empty($new_id)) { return (int)$new_id; }
        return (int)$fallback_user_id;
    }
}
...
if ($first == 'save-settings') {
    ...
    foreach ($_POST as $key => $settings_to_save) {
        //if ($key != 'ffmpeg_binary_file') {
            // Fix HTML entities in API URLs
            if (strpos($key, '_api_url') !== false) {
                $settings_to_save = html_entity_decode($settings_to_save, ENT_QUOTES, 'UTF-8');
            }
    ...
    if (!empty($submit_data)) {
        ...
        foreach ($submit_data as $key => $value) {
            ...
            if ($key == 'bank_description') {
                ...
                // Ensure success when key doesn't exist yet by inserting
                if (!$update) {
                    $exists = $db->where('name', $key)->getValue(T_CONFIG, 'COUNT(*)');
                    if (!$exists) {
                        $ins = $db->insert(T_CONFIG, array('name' => $key, 'value' => $value));
                        if ($ins) { $update = true; }
                    }
                }
            }
                ...
                if ($key != 'fav_category') {
        // Don't use PT_Secure for API URLs to prevent HTML entity encoding
        if (strpos($key, '_api_url') !== false) {
            $secure_value = $value; // Keep original value for API URLs
        } else {
            $secure_value = PT_Secure($value, 0);
        }
        $update = $db->where('name', $key)->update(T_CONFIG, array('value' => $secure_value));
                    // If the config key does not exist yet, insert it (allows new options like 'upload_concurrency')
                    $exists = $db->where('name', $key)->getValue(T_CONFIG, 'COUNT(*)');
                    if (!$exists) {
                        // Don't use PT_Secure for API URLs to prevent HTML entity encoding
                        if (strpos($key, '_api_url') !== false) {
                        ...
                        $ins = $db->insert(T_CONFIG, array('name' => $key, 'value' => $secure_value));
                        if ($ins) { $update = true; }
                    }
                }
            }
        ...
        if (isset($submit_data['oss_upload'])) {
            $get_config_json = file_get_contents('./nodejs/config.json');
            if (!empty($get_config_json)) {
                $config_json = json_decode($get_config_json);
                $config_json->amazon = ($submit_data['oss_upload'] == 'on') ? true : false;
                $config_json->amazon_bucket = $submit_data['oss_bucket_name'];
                $encode = json_encode($config_json, JSON_PRETTY_PRINT);
                $write_file = file_put_contents('./nodejs/config.json', $encode);
            }
        }
...
if ($first == 'testOSS') {
    ...
    try {
        // Create oss instance
        $accessKeyId = $pt->config->aliyun_oss_key;
        $accessKeySecret = $pt->config->aliyun_oss_s_key;
        $endpoint = $pt->config->aliyun_oss_endpoint;
        $ossDomain = $pt->config->aliyun_oss_domain;
        $ossClient = new OssClient($accessKeyId, $accessKeySecret, $endpoint);
        // Get all buckets
        $buckets = $ossClient->listBuckets();

        if (!empty($buckets)) {
            if ($ossClient->doesBucketExist($pt->config->oss_bucket_name)) {
                $data['status'] = 200;
                $array = array(
                    'upload/photos/d-cover.jpg',
                    'upload/photos/d-avatar.jpg',
...
if ($first == 'ffmpeg_debug') {
    if (!isfuncEnabled("shell_exec") || !isfuncEnabled("exec")) {
        $data['status'] = 200;
        $data['data']   = "The function: shell_exec, exec is not enabled, please contact your hosting provider to enable it, it's required for FFMPEG";
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }
...
if ($first == 'load-items-from-avdbapi') {
    if (!empty($_POST['currentPage'])) {
        $page = PT_Secure($_POST['currentPage']);
        $api_url_template = 'https://avdbapi.com/api.php/provide/vod/?ac=detail&pg=';
        if (!empty($_POST['apiUrl'])) {
            $api_url_template = $_POST['apiUrl'];
        }

        $data_list = array();
        $api_url = $api_url_template . $page;
        $response = connect_to_url($api_url);
        $result = json_decode($response, true);
        if (!empty($result['list'])) {
            foreach ($result['list'] as $item) {
                // Always include all items, let the import logic handle duplicates
                $data_list[] = $item;
            }
        }
        $data = array('status' => 200, 'items' => $data_list);
    } else {
        $data = array('status' => 400, 'error' => 'Missing parameters');
    }
}

// XVID API: list loader
if ($first == 'load-items-from-xvidapi') {
    if (!empty($_POST['currentPage'])) {
        $page = PT_Secure($_POST['currentPage']);
        $api_url_template = 'https://xvidapi.com/api.php/provide/vod/?ac=detail&pg=';
        if (!empty($_POST['apiUrl'])) {
            $api_url_template = $_POST['apiUrl'];
        }
...
// Server-side single item import for AVDB entries
if ($first == 'import-avdb-item') {
    $data = array('status' => 400);
    ...
    // Allow concurrent requests by releasing session lock
    if (session_status() === PHP_SESSION_ACTIVE) { @session_write_close(); }
    header('Content-Type: application/json; charset=utf-8');
    if (empty($_POST['item'])) {
        $data['message'] = 'missing item';
        echo json_encode($data); exit;
    }
    ...
    // Ensure external map table exists
    $db->rawQuery("CREATE TABLE IF NOT EXISTS `".T_EXTERNAL_MAP."` (
        `id` INT NOT NULL AUTO_INCREMENT,
        `provider` VARCHAR(32) NOT NULL,
        `remote_id` BIGINT NOT NULL,
        `movie_code` VARCHAR(64) DEFAULT '',
        `local_video_id` INT NOT NULL,
        `last_hash` VARCHAR(64) DEFAULT '',
        `updated_at` INT NOT NULL DEFAULT 0,
        PRIMARY KEY (`id`),
        UNIQUE KEY `uniq_provider_remote` (`provider`,`remote_id`),
        KEY `movie_code` (`movie_code`),
        KEY `local_video_id` (`local_video_id`)
    ...
    $actors = array();
    if (!empty($item['actor']) && is_array($item['actor'])) { $actors = $item['actor']; }
    $categories = array();
    if (!empty($item['category']) && is_array($item['category'])) { $categories = $item['category']; }
    $movie_code = '';
    if (!empty($item['movie_code'])) { $movie_code = str_replace('-', '', $item['movie_code']); }
    $poster_url = isset($item['poster_url']) ? $item['poster_url'] : '';
    
    // Check if we should download images to local
    $download_images = isset($_POST['download_images']) ? ($_POST['download_images'] === 'on') : false;
    if ($download_images && !empty($poster_url)) {
        $poster_url = downloadImageToLocal($poster_url, 'poster');
    }
    ...
    if (empty($title) || empty($embed_url)) {
        $data['message'] = 'missing title or embed url';
        echo json_encode($data); exit;
    }
    // Build tags
    $tags_arr = array();
    ...
    // Duplicate check via external map by provider/id or by movie_code
    $provider = 'avdb';
    ...
    // Normalize server-side like submit-imported-video
    $duration = '00:00';
    if (!empty($item['time'])) { $duration = $item['time']; }
    $video_id = PT_GenerateKey(15, 15);
    ...
    // Map AVDB type/category to numeric category id in T_LANGS; auto-create if missing
    $category_id = 0;
    $pick = '';
    ...
    if (!empty($pick)) {
        ...
        if (!empty($row) && !empty($row->id)) {
            ...
            // Create new category with english name; set lang_key to id
    ...
    // If still empty, ensure a default category exists
    ...
    $video_privacy = 0;
    $age_restriction = 1;
    // Determine owner by author if present
    $owner_id = $pt->user->id;
    if (!empty($item['author'])) { $owner_id = pt_author_to_user_id($item['author'], 'avdb', $owner_id); }
    $data_insert = array(
        'video_id' => $video_id,
        'user_id' => $owner_id,
        'title' => $title_clean,
    ...
    if (!empty($map) && !empty($map->local_video_id)) {
        ...
        if ($map->last_hash === $content_hash) {
            echo json_encode(array('status'=>200,'video_id'=>$map->local_video_id,'skipped'=>true,'reason'=>'no_changes','link'=>PT_Link('watch/'.$video_id))); exit;
        }
        
        // Update existing video if changed
        $update = array(
            'title' => $title_clean,
            'description' => $desc_clean,
            'tags' => PT_Secure($tags_str,1),
        ...
        echo json_encode(array('status'=>200,'video_id'=>$map->local_video_id,'updated'=>true,'link'=>PT_Link('watch/'.$video_id))); exit;
    } else {
        // Insert new and map
        ...
        if ($ok) {
            ...
            $db->insert(T_EXTERNAL_MAP,$map_data);
            echo json_encode(array('status'=>200,'video_id'=>$video_id,'link'=>PT_Link('watch/'.$video_id))); exit;
        } else {
            $err = method_exists($db,'getLastError') ? $db->getLastError() : '';
            header('Content-Type: application/json');
            echo json_encode(array('status'=>400,'message'=>'db_error: '.$err)); exit;
        }
    }
}

// XVID API: single item import
if ($first == 'import-xvid-item') {
    $data = array('status' => 400);
    if (!PT_IsAdmin()) {
        $data['message'] = 'forbidden';
        echo json_encode($data); exit;
    ...
    if (!empty($item['time'])) { $duration = $item['time']; }
    $video_id = PT_GenerateKey(15, 15);
    $video_title = PT_Secure(addToHashTags($title,1));
    if (function_exists('mb_substr')) { $video_title = mb_substr($video_title, 0, 100, 'UTF-8'); } else { $video_title = substr($video_title, 0, 100); }
    $video_description = PT_Secure(addToHashTags($desc,1));
    $stars = '';
    if (!empty($actors)) { $stars = implode(', ', $actors); }
    if (function_exists('mb_substr')) { $stars = mb_substr($stars, 0, 200, 'UTF-8'); } else { $stars = substr($stars, 0, 200); }
    $category_id = 0;
    ...
    // Determine owner by author if present
    $owner_id = $pt->user->id;
    if (!empty($item['author'])) { $owner_id = pt_author_to_user_id($item['author'], 'xvid', $owner_id); }
    $data_insert = array(
        'video_id' => $video_id,
        'user_id' => $owner_id,
        'title' => $video_title,
