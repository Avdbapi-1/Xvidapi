...
if (PT_IsAdmin() == false && $pt->user->is_pro != 1 && $pt->user->imports >= $max_import) {
...
// Decode any HTML entities coming from providers to avoid showing &amp; in UI
...
if (!empty($_POST['link'])) {
    ...
    if (!empty($_POST['video_id'])) {
        ...
        $videoData           = PT_GetVideoByID($videoData, 0, 0, 0, 0, 1);
    ...
    if (empty($error)) {
        ...
        // Final cleanup for UI
        $title = pt_decode_import_str($title);
        $description = pt_decode_import_str($description);
        $tags = pt_decode_import_str($tags);
        ...
        $data = array(
            ...
            'movie_release' => isset($_POST['movie_release']) ? $_POST['movie_release'] : "",
