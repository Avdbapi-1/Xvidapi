...
// Lightweight health check for server selector / external monitors
if (!empty($_GET['ping']) || (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'HEAD')) {
    ...
    if (!empty($_FILES['video']['tmp_name'])) {
        ...
        if (!empty($video)) {
            ...
            foreach ($img_pos as $i) {
                ...
                // Ensure STDERR is redirected correctly (2>&1) for cross-PHP/OS consistency
            ...
            $data['uploaded_id'] = $db->insert(T_UPLOADED, array(
                'user_id' => $pt->user->id,
                'path' => $filename,
                'time' => time()));
