...
elseif (!is_numeric($_POST['last_id']) || $_POST['last_id'] < 0) {
	...
	if ($type == 'subscriptions') {
        ...
        $video = $pt->video = PT_GetVideoByID($video, 0, 0, 0, 0, 1);
		        $pt->last_video = false;
		        if ($key == $len - 1) {
		            $pt->last_video = true;
		        }
        ...
        $video = PT_GetVideoByID($video, 0, 0, 0, 0, 1);
		        $pt->last_video = false;
		        if ($key == $len - 1) {
		            $pt->last_video = true;
		        }
   ...
   //              }
		...
		if (!empty($videos)) {
		    $len = count($videos);
		    foreach ($videos as $key => $video) {
        $video = PT_GetVideoByID($video, 0, 1, 0, 0, 1);
		        $pt->last_video = false;
		        if ($key == $len - 1) {
		            $pt->last_video = true;
		        }
    ...
    if ($last_video->is_movie || $_POST['paid_sort'] == 'rented_movies') {
            $rented_movies = " AND t.`type` != 'rent' ";
		    if ($_POST['paid_sort'] == 'rented_movies') {
                $rented_movies = " AND t.`type` = 'rent' ";
		    }
            $get = $db->rawQuery('SELECT t.* FROM '.T_VIDEOS_TRSNS.' t JOIN '.T_VIDEOS.' v ON v.id = t.video_id AND v.is_movie = 1 AND v.user_id NOT IN ('.implode(",", $pt->blocked_array).') WHERE t.paid_id >= '.$pt->user->id.' AND t.id < '.$tr_id.' '.$rented_movies.' ORDER BY t.id DESC LIMIT 20');
	 	}
	 	else{
            $rented_movies = " AND t.`type` != 'rent' ";
        ...
        $comments = $db->rawQuery('SELECT c.* FROM '.T_COMMENTS.' c JOIN '.T_VIDEOS.' v ON v.id = c.video_id AND v.user_id = '.$pt->user->id.' WHERE c.user_id NOT IN ('.implode(",", $pt->blocked_array).') AND c.id < '.$comment_id.' ORDER BY c.time DESC LIMIT 20');
		...
		if ($type == 'views') {
			...
			if ($sort_type == 'all_time') {
				...
				$videos = $db->rawQuery('SELECT u.user_id AS user_id, COUNT(*) AS count FROM '.T_VIEWS.' v ,'.T_VIDEOS.' u WHERE v.time >= '.$start.' AND v.time <= '.$end.' AND u.id = v.video_id AND  u.user_id NOT IN ('.$ids.') AND u.user_id NOT IN ('.implode(",", $pt->blocked_array).') GROUP BY u.user_id HAVING count <= '.$last_view.' ORDER BY count DESC LIMIT 20');
			}
