...
if (!empty($_GET['type_']) && $_GET['type_'] == 'add' && !empty($_SESSION['finger'])) {
    // Read session fingerprint then release the session lock
    $session_finger = $_SESSION['finger'];
	...
	if (!empty($video)) {
    	...
    	if ($add == true) {
    		...
    		$_data[] = $insert_data;
            // Client-only cookie is enough for lightweight dedupe; avoid session writes
