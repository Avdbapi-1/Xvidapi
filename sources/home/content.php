...
// Microcache the home page for anonymous users
if (IS_LOGGED == false) {
    $home_cache_key = 'home_page_v1';
    $cached = PT_CacheGet($home_cache_key, array(), 60);
    if (!empty($cached)) {
        $pt->content = $cached;
        return;
    }
...
$video_obj = $db->where('featured', '1')
                ->where('user_id',$pt->blocked_array , 'NOT IN')
                ->where('is_movie', 0)
                ->where('privacy', 0)
                ->where('live_time', 0)
                ->where('approved',1)
                ...
                ->orderBy('id', 'DESC')
                ->getOne(T_VIDEOS);
$get_video = PT_GetVideoByID($video_obj, 0, 1, 0, 0, 0);

if (empty($get_video)) {

    $db->where('converted', '2','<>');
    $get_video = PT_GetVideoByID($db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie', 0)->where('live_time', 0)->where('approved',1)->where('is_short', 0)->orderBy('id', 'DESC')->getOne(T_VIDEOS), 0, 1, 0, 0, 0);
}
...
if (IS_LOGGED == false && !empty($home_cache_key)) {
    PT_CacheSet($home_cache_key, array(), $pt->content);
}
