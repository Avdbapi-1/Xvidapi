...
// Anonymous page microcache (speeds TTFB to sub-second)
if (IS_LOGGED == false && empty($_GET['feed'])) {
    $cache_args = array('page'=>$page,'page_id'=>$pt->page_number);
    if (!empty($_GET['type'])) { $cache_args['type'] = $_GET['type']; }
    if (!empty($_GET['id'])) { $cache_args['cat'] = $_GET['id']; }
    if (!empty($_GET['sub_id'])) { $cache_args['sub'] = $_GET['sub_id']; }
    if (!empty($_GET['keyword'])) { $cache_args['kw'] = (string)$_GET['keyword']; }
    if (!empty($_GET['min_price'])) { $cache_args['minp'] = (int)$_GET['min_price']; }
    if (!empty($_GET['max_price'])) { $cache_args['maxp'] = (int)$_GET['max_price']; }
    if (!empty($_GET['license'])) { $cache_args['lic'] = (string)$_GET['license']; }
    $videos_cache_key = 'videos_page_' . json_encode($cache_args);
    $ttl = 60;
    if ($page == 'top' && (!empty($_GET['type']) && $_GET['type'] == 'all_time')) { $ttl = 300; }
    $cached = PT_CacheGet($videos_cache_key, array(), $ttl);
    if (!empty($cached)) {
        $pt->content = $cached;
        return;
    }
...
// Store microcache for anonymous users
if (IS_LOGGED == false && empty($_GET['feed']) && !empty($videos_cache_key)) {
    PT_CacheSet($videos_cache_key, array(), $pt->content);
}
