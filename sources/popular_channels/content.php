...
elseif ($sort_type == 'this_month') {
	...
	// Avoid cal_days_in_month dependency; use date('t') for last day of month
...
// Lightweight caching for guests (HTML + count), keyed by type + window
$cache_used = false; $videos_count = 0;
if (!defined('IS_LOGGED') || IS_LOGGED == false) {
    $cache_key_base = 'popular_channels_' . $type . '_' . $sort_type;
    $cached_html = PT_CacheGet($cache_key_base, [], 180);
    $cached_count = PT_CacheGet($cache_key_base . '_count', [], 180);
    if ($cached_html !== false && $cached_count !== false) {
        $final2 = $cached_html;
        $videos_count = (int)$cached_count;
        $cache_used = true;
    }
}

if (!$cache_used && $type == 'views') {
	if ($sort_type == 'all_time') {
		$videos = $db->rawQuery('SELECT user_id, SUM(views) AS count FROM '.T_VIDEOS.' WHERE user_id NOT IN ('.implode(",", $pt->blocked_array).') GROUP BY user_id ORDER BY count DESC LIMIT 20');
    		...
    		$videos = $db->rawQuery('SELECT u.user_id AS user_id, COUNT(*) AS count FROM '.T_VIEWS.' v ,'.T_VIDEOS.' u WHERE v.time >= '.$start.' AND v.time <= '.$end.' AND u.id = v.video_id AND u.user_id NOT IN ('.implode(",", $pt->blocked_array).') GROUP BY u.user_id ORDER BY count DESC LIMIT 20');
	}
}
elseif (!$cache_used && $type == 'subscribers') {
	if ($sort_type == 'all_time') {
...
elseif (!$cache_used && $type == 'most_active') {
	$time = strtotime(date('l').", ".date('M')." ".date('d').", ".date('Y'));
...
if ($pt->config->theme == 'default') {
    // Be defensive: if anything fails here, don't break the page
    try {
        if (function_exists('getTopChannelsForThisMonth')) {
            $topChannels = getTopChannelsForThisMonth();
            if (is_array($topChannels)) {
                $featured = isset($topChannels['featured']) ? $topChannels['featured'] : '';
                $featuredIds = isset($topChannels['featuredIds']) ? $topChannels['featuredIds'] : [];
            }
        }
    } catch (Throwable $e) {
        // Log and continue gracefully
        @file_put_contents(__DIR__ . '/../../logs/popular_channels_error.log',
            '['.date('c')."] default featured error: ".$e->getMessage()."\n",
            FILE_APPEND
        );
        $featured = '';
        $featuredIds = [];
...
if (!$cache_used && !empty($videos)) {
	foreach ($videos as $key => $value) {
    	...
    	if (!empty($user) && !in_array($user->id, $featuredIds)) {
	    	...
	    	$pt->subs = [];
        if ($pt->config->theme == 'default') {
            // Avoid ORDER BY RAND(); pick randomly from recent subscribers
            $recent = $db->where('user_id',$user->id)->orderBy('id','DESC')->get(T_SUBSCRIPTIONS,30);
            if (!empty($recent)) {
                $pick = array();
                $idxs = array_rand($recent, min(3, count($recent)));
                if (!is_array($idxs)) { $idxs = array($idxs); }
                foreach ($idxs as $ix) { $pick[] = $recent[$ix]; }
    ...
    // Save guest cache
    if (!defined('IS_LOGGED') || IS_LOGGED == false) {
        PT_CacheSet($cache_key_base, [], $final2);
        PT_CacheSet($cache_key_base . '_count', [], (string)$videos_count);
    }
}
...
$pt->type = $type;
$pt->channels_count = $videos_count;
