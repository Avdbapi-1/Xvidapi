<?php
// perf timing
$__t0 = microtime(true); $__last = $__t0; $__perf = array();
...
// Anonymous microcache for AJAX loads of watch page content
// Keeps TTFB low by serving pre-rendered HTML for non-logged visitors
if (IS_LOGGED == false && !empty($pt->is_ajax_load)) {
    $watch_cache_key = 'watch_page_v1_' . $id;
    $cached = PT_CacheGet($watch_cache_key, array(), 120);
    if (!empty($cached)) {
        $pt->page = 'watch';
        $pt->content = $cached;
        return;
...
// Small microcache for direct (non-AJAX) anonymous loads
if (IS_LOGGED == false && empty($pt->is_ajax_load)) {
    $watch_cache_key = 'watch_page_v2_' . $id;
    $cached = PT_CacheGet($watch_cache_key, array(), 60);
    if (!empty($cached)) {
        $pt->page = 'watch';
        $pt->content = $cached;
        return;
...
$__now = microtime(true); $__perf['get_video_ms'] = (int)(($__now - $__last) * 1000); $__last = $__now;
...
$__now = microtime(true); $__perf['next_up_ms'] = (int)(($__now - $__last) * 1000); $__last = $__now;

foreach ($next_up_videos as $key => $next_up_video) {
    $nextVideo  = PT_GetVideoByID($next_up_video, 0, 0, 0, 0, 1);
...
$__now = microtime(true); $__perf['related_ms'] = (int)(($__now - $__last) * 1000); $__last = $__now;
...
if ($pt->is_wl === true && IS_LOGGED === true) {
    ...
    foreach ($wl_list_videos as $key => $pl_vid) {
        $pl_vid         = PT_GetVideoByID($pl_vid, 0, 0, 0, 0, 1);
...
foreach ($related_videos as $key => $related_video) {
    if (empty($next_up_videos) || (!empty($next_up_videos) && !empty($next_up_videos[0]) && $next_up_videos[0]->id != $related_video->id)) {
        $related_video  = PT_GetVideoByID($related_video, 0, 0, 0, 0, 1);
        if (empty($next_video)) {
            $next_video = PT_LoadPage('watch/video-sidebar', array(
                'ID' => $related_video->id,
...
// Defer comments rendering to AJAX to improve initial TTFB
$comments = '';
...
if ($last_ads == 0 && !$is_pro && $ads_sys) {
    // Remove Video Ads (T_VIDEO_ADS) and keep only User Ads
    if ($vid_monit) {
...
$__now = microtime(true); $__perf['ads_ms'] = (int)(($__now - $__last) * 1000); $__last = $__now;
...
// Store microcache only for anonymous AJAX loads (page_loading.php)
if (IS_LOGGED == false && !empty($pt->is_ajax_load) && !empty($watch_cache_key)) {
    PT_CacheSet($watch_cache_key, array(), $pt->content);
}

// Also microcache direct (non-AJAX) anonymous loads
if (IS_LOGGED == false && empty($pt->is_ajax_load) && !empty($watch_cache_key)) {
    PT_CacheSet($watch_cache_key, array(), $pt->content);
}

// Perf log
$__total = (int)((microtime(true) - $__t0) * 1000);
$__perf['total_ms'] = $__total;
PT_PerfLog('watch', array(
    'video_id' => isset($get_video->id) ? $get_video->id : 0,
    'short_id' => $id,
    'user_id'  => (IS_LOGGED && !empty($user->id)) ? $user->id : 0,
    'steps'    => $__perf
));
//
